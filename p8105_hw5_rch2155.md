...
tibble with a normal distribution created and a ttest function is
included to be applied to the following code that uses the funciton.
...
and create a data frame for the ttest function when mu includes the
...
The following code creates to plots and binds them together. The first
...
of mu (shown on the left). There are visable deviations from the average
